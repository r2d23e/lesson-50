...
const setup = (bank) => {
    ...
    for (let item of bank) {
        ...
        for (let second of item.expensesPerYear) {
            item.expensesPerMonth += second.total / 12
        ...
        procentch = item.budget - item.budget / 100 * item.percent
        // unSuccessful = bank.filter(item => item.budget > procentch)
